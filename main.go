...
func (t Training) meanSpeed() float64 {
	...
	if t.Duration.Hours() > 0 {
		totalMeanSpeed = t.distance() / t.Duration.Hours()
	}
...
func (w Walking) Calories() float64 {
	convertedMeanSpeed := w.meanSpeed() * KmHInMsec
	squaredMeanSpeed := math.Pow(convertedMeanSpeed, 2)
	return (CaloriesWeightMultiplier*w.Weight + (squaredMeanSpeed/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	...
	if s.Duration.Hours() > 0 {
		totalMeanSpeed = float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()
	}
	return totalMeanSpeed
}
